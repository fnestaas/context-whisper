...
def small_config() -> ContextWhisperConfig:
    return ContextWhisperConfig(
        d_model=64, decoder_attention_heads=4, text_encoder_attention_heads=4
    )
...
class TestConfig:
    def test_random_args(self, config_args: dict):
        ...
        kwargs = {k: get_value(v) for k, v in config_args.items()}
...
class TestPretrained:
    def test_hasgrad(self, small_config: Config):
        ...
        module = model.get_text_encoder()
        assert module.requires_grad_()
        module = model.get_spectrogram_encoder()
        assert module.requires_grad_()
        module = model.get_decoder()
        assert module.requires_grad_()
...
class TestDecoder:
    ...
    # TODO: test layer
    @pytest.mark.parametrize("size_id", ["small", "default", "pretrained"])
    def test_basic_inputs(
        self,
        size_id: str,
        request: pytest.FixtureRequest,
...
class TestModel:
    ...
    def test_freeze_module(
        ...
        def get_ref_ids(model, key, *dims, require_grad: bool):
            ...
            for p in params:
                if (
                    p.dim() == 2
                    and all([s >= max(d) for s, d in zip(p.shape, dims)])
                    and p.requires_grad == require_grad
                ):
                    retval = p[dims[0], dims[1]].detach()
                    retval.requires_grad = False
                    return retval
            raise ValueError(
                f"did not find an appropriate parameter for {key=}. It has {sum([p.requires_grad for p in module.parameters()])} parameters with grad."
            )
        ...
        n_requires_grad_module = sum(
            [p.requires_grad for p in getattr(model, module).parameters()]
        )
        assert n_requires_grad_model == n_requires_grad_module
        assert n_requires_grad_model > 0
        assert all(
            [
                all([not p.requires_grad for p in getattr(model, m).parameters()])
                for m in frozen_modules_str
            ]
        )
        ...
        outputs = model.forward(
            ...
            text_encoder_inputs_embeds=small_token_emb_input
            if module == "spectrogram_encoder"
            else None,
            spectrogram_input_features=spectrogram_input
            if module != "decoder"
            else None,
        ...
        loss = loss_fn(
            outputs.last_hidden_state, torch.rand(*outputs.last_hidden_state.shape)
    ...
    @pytest.mark.parametrize(
        [
            "pretrained_bert",
            "pretrained_whisper",
            "pretrained_whisper_decoder_only",
            "pretrained_whisper_causal_lm",
        ],
        [
    ...
    def test_pretrained(
        ...
        pretrained_whisper_causal_lm: bool,  # use a projection on top of the context whisper model to get logits
    ):
        """
        ...
        def model2params(m: torch.nn.Module):
            return torch.concat([p.flatten() for p in m.parameters()])
        ...
        if pretrained_whisper or pretrained_whisper_causal_lm:
            whisper_pretrained_str = "openai/whisper-small"
        else:
        ...
        if not pretrained_whisper_causal_lm:
            model = Model(config)
        else:
            model = ContextWhisperForCausalLM(config)
        if pretrained_bert:
            bert = BertModel.from_pretrained("google-bert/bert-base-uncased")
            model_eq(bert, model.get_text_encoder())
        if (
            pretrained_whisper
            or pretrained_whisper_decoder_only
            or pretrained_whisper_causal_lm
        ):
            # check decoder
            if not pretrained_whisper_causal_lm:
                ...
                whisper = WhisperForCausalLM.from_pretrained("openai/whisper-small")
                assert isinstance(whisper, WhisperForCausalLM)
    ...
    def test_basic_inputs(self, pretrained_model: Model):
        ...
        with torch.no_grad():
            ...
            model_out = pretrained_model.forward(
                ...
                text_encoder_input_ids=torch.tensor(
                    text_encoder_tokens["input_ids"]
                ).unsqueeze(0),
                decoder_input_ids=torch.tensor(tokens["input_ids"]).unsqueeze(0),
            )
            assert model_out.last_hidden_state.shape == (
                1,
